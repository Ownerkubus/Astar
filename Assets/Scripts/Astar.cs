...
public class Astar
{
    ...
    public List<Vector2Int> FindPathToTarget(Vector2Int startPos, Vector2Int endPos, Cell[,] grid)
    {
        ...
        while (openNodes.Count > 0)
        {
            ...
            List<Node> neighbours = GetAvailableNeighbours(currentNode, grid.GetLength(0), grid.GetLength(1));
            foreach (Node neighbour in neighbours)
            {
                ...
                if (tentativeGScore < neighbour.GScore || !openNodes.Contains(neighbour))
                {
                    ...
                    if (IsAccessible(currentCell, neighbourCell) && !openNodes.Contains(neighbour))
                    {
    ...
    private List<Node> GetAvailableNeighbours(Node startNode, int width, int height)
    {
    ...
    private bool IsAccessible(Cell startCell, Cell neighbour)
    {
        if (neighbour.gridPosition.y < startCell.gridPosition.y && neighbour.HasWall(Wall.UP))
        {
            return false;
        }
        ...
        else if (neighbour.gridPosition.x > startCell.gridPosition.x && neighbour.HasWall(Wall.LEFT))
        {
            return false;
        }
